...
- For vault-backed remote editing, also set:
  - `SIDENOTE2_DGX_VAULT_ROOT=/absolute/path/to/synced-vault`
  - the mobile client now sends `vaultRelativePath` plus a `sha256` note hash, and the bridge rejects stale DGX copies before Codex starts
- For actual mobile, WAN, or VPN use, the plugin still expects an HTTPS bridge URL. The bridge runner now supports direct TLS with:
  - `SIDENOTE2_DGX_TLS_CERT_FILE`
  - `SIDENOTE2_DGX_TLS_KEY_FILE`
  - optional `SIDENOTE2_DGX_TLS_CA_FILE`
- If you already terminate TLS in nginx, Caddy, or another reverse proxy, you can keep the bridge itself on HTTP behind that proxy.
- A shared consumer-VPN exit IP is not enough by itself. The phone still needs a real network path to the DGX bridge, such as a routed private VPN, Tailscale, or a reverse proxy that forwards to the DGX host.
...
- When `SIDENOTE2_DGX_VAULT_ROOT` is configured and the request uses `workspace-aware-vault`, the bridge resolves the active note under that vault root, verifies the synced copy hash, and starts Codex from the note's nearest repo root or containing folder.
