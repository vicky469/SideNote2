...
test("buildAllCommentsNoteContent groups comments by file and shows selected text and comment", () => {
    ...
    assert.doesNotMatch(content, /## Thought Trail/);
    assert.doesNotMatch(content, /Page notes/);
    assert.doesNotMatch(content, /Anchored notes/);
    assert.match(content, /\*\*A\.md\*\*[\s\S]*<span class="sidenote2-index-kind-dot sidenote2-index-kind-anchored"><\/span> \[~~alpha~~\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=A\.md&commentId=comment-2&kind=anchored\)/);
    assert.match(content, /\*\*B\.md\*\*[\s\S]*<span class="sidenote2-index-kind-dot sidenote2-index-kind-anchored"><\/span> \[beta\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=B\.md&commentId=comment-1&kind=anchored\)/);
    assert.doesNotMatch(content, /Comment A/);
    assert.ok(content.indexOf("**A.md**") < content.indexOf("**B.md**"));
});
...
test("buildAllCommentsNoteContent labels page notes and orphaned notes", () => {
    ...
    assert.equal(content.match(/\*\*Folder\/Note\.md\*\*/g)?.length ?? 0, 1);
    assert.match(content, /\*\*Folder\/Note\.md\*\*[\s\S]*<span class="sidenote2-index-kind-dot sidenote2-index-kind-page"><\/span> \[page note · 1\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=Folder%2FNote\.md&commentId=page-note&kind=page\)\n<span class="sidenote2-index-kind-dot sidenote2-index-kind-anchored"><\/span> \[orphaned · missing text\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=Folder%2FNote\.md&commentId=orphan-note&kind=anchored\)/);
});
...
test("buildAllCommentsNoteContent numbers page notes per file when no mentioned page label is available", () => {
    ...
    assert.match(content, /\*\*Folder\/Note\.md\*\*[\s\S]*\[page note · 1\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=Folder%2FNote\.md&commentId=page-note-1&kind=page\)/);
    assert.match(content, /\*\*Folder\/Note\.md\*\*[\s\S]*\[page note · 2\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=Folder%2FNote\.md&commentId=page-note-2&kind=page\)/);
    assert.match(content, /\*\*Folder\/Other\.md\*\*[\s\S]*\[page note · 1\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=Folder%2FOther\.md&commentId=page-note-other-file&kind=page\)/);
});
...
test("buildAllCommentsNoteContent expands mentioned pages and collapses duplicates per comment", () => {
    ...
    assert.match(content, /<span class="sidenote2-index-kind-dot sidenote2-index-kind-page"><\/span> \[Another page\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=Folder%2FNote\.md&commentId=page-note&kind=page\)/);
    assert.match(content, /<span class="sidenote2-index-kind-dot sidenote2-index-kind-page"><\/span> \[Third page\]\(obsidian:\/\/side-note2-comment\?vault=dev&file=Folder%2FNote\.md&commentId=page-note&kind=page\)/);
    ...
    assert.equal(content.match(/\[Another page]/g)?.length ?? 0, 1);
});
