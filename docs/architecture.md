...
- `src/control/*` owns entry, mutation, session, workspace, workspace view, lifecycle, registration, navigation, highlight, persistence, and index settings controllers.
...
- `src/ui/*` handles the sidebar view, draft/persisted/interaction helpers, editor helpers, settings, and modals.
...
- Middle: the coordination and decision layer.
- Bottom: persistence, retargeting, and derived views.
...
This is a separate canvas so you can zoom the flow without crowding the main architecture board.
...
| Index note is stale or wrong | `src/index/AggregateCommentIndex.ts`, `src/core/derived/allCommentsNote.ts`, `src/cache/ParsedNoteCache.ts` |
